...
class AriController extends EventEmitter {
	...
	async connect() {
		...
		// second channel
		this.secondChannel = this.ari.Channel();
		this.secondChannel.on('StasisStart', (event, chan) => {
			this.bridge.addChannel({channel: chan.id});
		});
		this.secondChannel.on('StasisEnd', (event, chan) => {
			this.close();
		});

		// Call the phone or confbridge specified in dialstring
		try {
			await this.secondChannel.originate({
				endpoint: "PJSIP/85294587533@gwsiptrunk", formats: this.options.format, app: "externalMedia",
			});
		} catch (error) {
			this.close();
		}
